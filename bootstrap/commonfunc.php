...
function compare($first, $second) {
	if ($first["settle_date"] == $second["settle_date"]) {
		return 0;
	}
	return ($first["settle_date"] < $second["settle_date"]) ? -1 : 1;
}
...
function _objectToArray ($object) {
	...
	return array_map('_objectToArray', (array) $object);
}

function _convertDateFormat($date, $format) {
	if(strtotime($date) == 0) return '0000-00-00';
