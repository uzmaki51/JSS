...
{
	// 2017-04-10 12:34:43형의 날자형문자렬을 2017-04-10 12시34분 으로 변환한다.
	function convert_datetime($datestr)
	{
		if(is_null($datestr) || empty($datestr))
			return '';

		$datetime = date_create($datestr);
		$returnStr = date_format($datetime, "Y年m月d日").'<br>'.date_format($datetime, 'H点i分');

		return $returnStr;
	}
}

if(!function_exists('convert_datetime_origin'))
{
	// 2017-04-10 12:34:43형의 날자형문자렬을 2017-04-10 12시34분 으로 변환한다.
	function convert_datetime_origin($datestr)
	{
		if(is_null($datestr) || empty($datestr))
			return '';

		$datetime = date_create($datestr);
		$returnStr = date_format($datetime, 'Y/m/d H:i');

		return $returnStr;
	}
}

if(!function_exists('convert_datestr'))
{
	function convert_datestr($dateStr)
	{
		if(is_null($dateStr))
			return '';
		...
		$returnStr = date_format($datetime, "Y年m月d日 H点i分");

		return $returnStr;
	}
}

if(!function_exists('show_string'))
{
