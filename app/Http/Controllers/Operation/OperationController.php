...
class OperationController extends Controller
{
    public function __construct()
    {
        ...
        if ($admin > 0) {
            $menulist = Menu::where('parentId', '=', '6')->orderBy('id')->get();
			...
			$user = Auth::user();
			if(in_array(6, explode(',', $user['menu']))) {
				$menulist = Menu::where('parentId', '=', '6')->where('admin', '=', '0')->get();
				foreach ($menulist as $menu) {
					$menuId = $menu['id'];
					$submenus = Menu::where('parentId', '=', $menuId)->orderBy('id')->get();
					$menu['submenu'] = $submenus;
				...
				$menulist = Menu::where('parentId', '=', '6')->where('admin', '=', '0')->whereIn('id', explode(',', $user['menu']))->get();
				foreach ($menulist as $menu) {
					$menuId = $menu['id'];
					$submenus = Menu::where('parentId', '=', $menuId)->orderBy('id')->get();
					$menu['submenu'] = $submenus;
