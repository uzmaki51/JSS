        ...
        <div class="page-content">
            ...
            <div class="page-header">
                <div class="col-md-8">
                    <h4>
                        <small>
                            {{$info->getShipNameAndPos().' ' .$info->realname}} {{$year}}{{transBusinessManage("title.MemberMonthAttendPrint")}}
                        </small>
                    </h4>
                </div>
            </div>
                ...
                <div class="row">
                    <table class="table table-bordered table-hover">
                        ...
                        @for($index = 0; $index<16; $index++)
                            ...
                            ?>
                            <tr>
                                <td style="text-align:center;width:3.8%">@if($fday != 15){{ $fday + 1}}{{transBusinessManage("captions.day")}}@endif</td>
                                <td style="text-align:center;width:3.8%">{{ $sday + 1}}{{transBusinessManage("captions.day")}}</td>
                                @for($month = 1;$month<13;$month++)
                                    ...
                                    <td style="width:3.8%;text-align: center;" @if($isfRest) class="rest" @endif>
                                        @if($fday != 15)
                                            <a @if(!empty($firstAttendMemo)) class="hide-option" title="{{ $firstAttendMemo }}" @endif>
                                                <?php $firstUrl = \App\Http\Controllers\Util::getStampUrl($firstAttendStatus, $signPath, 2); ?>
                                                @if($firstUrl != null)
                                                    <img src="{{$firstUrl}}" style="width: 25px;height: 15px;">
                                                @else
                                                    {{$firstAttendStatus}}
                                                @endif
                                            </a>
                                    ...
                                    <td style="width:3.8%;text-align: center;" @if($issRest) class="rest" @endif>
                                        <a @if(!empty($secondAttendMemo)) class="hide-option" title="{{ $secondAttendMemo }}" @endif>
                                            <?php $secondUrl = \App\Http\Controllers\Util::getStampUrl($secondAttendStatus, $signPath, 2); ?>
                                            @if($secondUrl != null)
                                                <img src="{{$secondUrl}}" style="width: 25px;height: 15px;">
                                            @else
                                                {{$secondAttendStatus}}
                                            @endif
                                        </a>
    ...
    <script>
        ...
        $(function () {
            ...
            $('.search-btn').on('click', function () {
                ...
                location.href = 'shipMemberYearReport?userId='+ memberId +'&year='+selyear;
            });
